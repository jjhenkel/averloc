...
def calc_attributions(model, data, output_fname):
    ...
    with open(os.path.join(opt.output_dir,'%s_%s_attributions.txt'%(output_fname, opt.data_path.replace('/','_')[:-4])), 'w') as f:
...
def evaluate_model(evaluator, model, data, save=False, output_dir=None, output_fname=None, src_field_name='src', get_attributions=False):
    ...
    if save:
        with open(os.path.join(output_dir,'%s_%s_preds.txt'%(output_fname, opt.data_path.replace('/','_')[:-4])), 'w') as f:
           f.writelines([a+'\n' for a in d['output_seqs']])
        with open(os.path.join(output_dir,'%s_%s_true.txt'%(output_fname, opt.data_path.replace('/','_')[:-4])), 'w') as f:
            f.writelines([a+'\n' for a in d['ground_truths']])
        with open(os.path.join(output_dir,'%s_%s_stats.txt'%(output_fname, opt.data_path.replace('/','_')[:-4])), 'w') as f:
            try:
